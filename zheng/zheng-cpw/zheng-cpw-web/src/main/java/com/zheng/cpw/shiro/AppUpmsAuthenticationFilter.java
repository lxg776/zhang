 ...
 */
public class AppUpmsAuthenticationFilter extends AuthenticationFilter {

    private final static Logger _log = LoggerFactory.getLogger(AppUpmsAuthenticationFilter.class);

    // 局部会话key
    private final static String ZHENG_UPMS_CLIENT_SESSION_ID = "zheng-cpw-client-session-id";
    // 单点同一个code所有局部会话key
    private final static String ZHENG_UPMS_CLIENT_SESSION_IDS = "zheng-cpw-client-session-ids";
    ...
    private boolean validateClient(ServletRequest request, ServletResponse response) {
        ...
        if (StringUtils.isNotBlank(cacheClientSession)) {
            ...
            jedis.close();
            // 移除url中的code参数
            if (null != request.getParameter("app_token")) {
        ...
        // 判断是否有认证中心code
        String code = request.getParameter("app_token");
        // 已拿到code
